...
const LoginForm = () => {
  ...
  const { setIsLoggedIn, setAdmin }: any = UserAuth();
  const {
    register,
    handleSubmit,
    setError,
  ...
  const onSubmit: SubmitHandler<LoginValues> = async (info) => {
    try {
      const UserInfo = {
        email: info.email,
    ...
    <div className="w-full">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full pb-2 pt-6">
          <Button className="w-full" title={"Login"} loading={isSubmitting} />
        </div>

        <div className="w-full pt-2 pb-0.5">
          Don't have an account?{" "}
          <Link
            to={authPath + "register"}
            className="underline cap text-blue-800 font-semibold hover:font-bold transition-all"
          >
            register
          </Link>
        </div>
